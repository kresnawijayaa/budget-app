...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'config_versions_gas_fill_interval_positive') THEN
    ALTER TABLE config_versions ADD CONSTRAINT config_versions_gas_fill_interval_positive CHECK (gas_fill_interval_days > 0) NOT VALID;
  END IF;
